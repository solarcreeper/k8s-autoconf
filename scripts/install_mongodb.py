...
yaml_path = os.path.join(os.path.abspath(os.path.join(os.getcwd(), '..')), 'yaml', 'mongodb')
configmap_path = os.path.join(yaml_path, 'mongo.configmap.yaml')
...
def install(namespace='test'):
    ...
    for p in [configmap_path, service_path, deploy_path]:
...
def uninstall(namespace='test'):
    for p in [deploy_path, service_path, configmap_path]:
        with open(yaml_data, 'w') as f:
            with open(p, 'r') as d:
                content = d.read()
                new_content = content.replace('namespace_template', namespace)
